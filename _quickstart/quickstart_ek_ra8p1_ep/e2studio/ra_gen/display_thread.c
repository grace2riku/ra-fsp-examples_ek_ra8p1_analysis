...
static StaticTask_t display_thread_memory;
                ...
                static uint8_t display_thread_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t display_thread_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.display_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void display_thread_create(void)
{
    ...
    display_thread = xTaskCreateStatic (
                    ...
                    #endif
                                        display_thread_func,
                                        (const char*) "Display Thread", 2048 / 4, // In words, not bytes
